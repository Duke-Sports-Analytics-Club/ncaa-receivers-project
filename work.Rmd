...
stats_2014 <- read.csv("data_pff_receiver/2014_receiver_pff_data.csv") %>% 
  select(-c(avg_depth_of_target, avoided_tackles, contested_catch_rate, 
            contested_receptions, contested_targets, declined_penalties, drops, 
...
stats_2020 <- read.csv("data_pff_receiver/2020_receiver_pff_data.csv") %>% 
  select(-c(avg_depth_of_target, avoided_tackles, contested_catch_rate, 
            ...
            interceptions, longest, pass_block_rate, pass_blocks, penalties, 
            slot_rate, slot_snaps, wide_rate, wide_snaps))
pro_day <- read.csv("data/prodaystats.csv")
```
...
dc2021_stats2020 <- stats_2020 %>% 
  filter(player %in% dc_2021) %>% 
  left_join(stats_2019, by = c("player", "player_id", "position"), 
            suffix = c("_draft", "_pre")) |>
  mutate(total_yards = yards_draft + yards_pre,
         total_catch_percent = (receptions_draft + receptions_pre) / (targets_draft + targets_pre),
         total_target_percent = (targets_draft + targets_pre) / (routes_draft + routes_pre),
         total_route_percent = (routes_draft + routes_pre) / (pass_plays_draft + pass_plays_pre),
         total_yprr = yprr_draft + yprr_pre,
         total_first_downs = first_downs_draft+first_downs_pre,
         total_yards_per_reception = yards_per_reception_draft + yards_per_reception_pre,
         total_yards_after_catch_per_reception = yards_after_catch_per_reception_draft + yards_after_catch_per_reception_pre,
         total_player_game_count = player_game_count_draft + player_game_count_pre,
         total_touchdowns = touchdowns_draft + touchdowns_pre,
         total_yards = yards_draft+yards_pre,
         total_yards_after_catch = yards_after_catch_draft + yards_after_catch_pre,
         total_targets = targets_draft+targets_pre,
         total_targeted_qb_rating = (targeted_qb_rating_draft+targeted_qb_rating_pre)/2,
         grades_hands_drop = grades_hands_drop_draft+grades_hands_drop_pre,
         grades_hands_fumble = grades_hands_fumble_draft+grades_hands_fumble_pre,
         grades_offense = grades_offense_draft+grades_offense_pre,
         grades_pass_route = grades_pass_route_draft+grades_pass_route_pre
         )
...
dc2018_stats2017 <- stats_2017 %>% 
  filter(player %in% dc_2018 | player %in% dc_2018_2) %>% 
...
dc2017_stats2016 <- stats_2016 %>% 
  filter(player %in% dc_2017 | player %in% dc_2017_2) %>% 
...
```{r}
joined_data <- inner_join(long_data, pro_day, 
                          by = c("player" = "Name", "position" = "POS")) %>%
  select(-c(Wonderlic, College))
