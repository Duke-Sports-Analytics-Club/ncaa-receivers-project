...
dc_2021_stats <- inner_join(dc_2021_pro_stats, dc2021_stats2020, by = c("player_id", "player", "position"))
...
dc2018_stats2017 <- stats_2017 %>% 
  ...
  rowwise() %>% 
  mutate(total_yards = yards_draft + yards_pre,
         total_catch_percent = sum(c(receptions_draft + receptions_pre), na.rm=TRUE) / sum(c(targets_draft + targets_pre), na.rm = TRUE),
         total_target_percent = sum(c(targets_draft + targets_pre), na.rm = TRUE) / sum(c(routes_draft + routes_pre), na.rm = TRUE),
         total_route_percent = sum(c(routes_draft + routes_pre), na.rm = TRUE) / sum(c(pass_plays_draft + pass_plays_pre), na.rm = TRUE),
         total_drop_percent = sum(c(drops_draft,drops_pre), na.rm = TRUE) / sum(c(targets_draft,targets_pre), na.rm = TRUE),
         total_yprr = yprr_draft + yprr_pre,
         total_first_downs = sum(c(first_downs_draft,first_downs_pre), na.rm = TRUE),
         total_yards_per_reception = sum(c(yards_per_reception_draft,yards_per_reception_pre), na.rm = TRUE),
...
dc2017_stats2016 <- stats_2016 %>% 
  ...
  mutate(total_yards = yards_draft + yards_pre,
         ...
         total_yards_per_reception = sum(c(yards_per_reception_draft,yards_per_reception_pre), na.rm = TRUE),
         total_yards_after_catch_per_reception = sum(c(yards_after_catch_per_reception_draft,
                                                       yards_after_catch_per_reception_pre), na.rm = TRUE) / 2,
         total_player_game_count = sum(c(player_game_count_draft,player_game_count_pre), na.rm = TRUE),
         total_touchdowns = sum(c(touchdowns_draft,touchdowns_pre), na.rm = TRUE),
         total_yards_per_game = ((sum(c(yards_draft,yards_pre), na.rm = TRUE)) / total_player_game_count), 
         total_yards_after_catch_per_game = ((sum(c(yards_after_catch_draft, yards_after_catch_pre), na.rm = TRUE)) /
                                               total_player_game_count), 
         total_targets_per_game = ((sum(c(targets_draft,targets_pre), na.rm = TRUE)) / total_player_game_count), 
         total_targeted_qb_rating = sum(c(targeted_qb_rating_draft,targeted_qb_rating_pre), na.rm = TRUE) ,
         games_draft = (player_game_count_draft / sum(c(player_game_count_draft, player_game_count_pre), na.rm = TRUE)),
         grades_hands_drop = sum(c((games_draft*grades_hands_drop_draft),((1-games_draft)*grades_hands_drop_pre)), 
                                 na.rm = TRUE),
         grades_hands_fumble = sum(c((games_draft*grades_hands_fumble_draft),((1-games_draft)*grades_hands_fumble_pre)),
...
```{r}
long_data <- rbind(dc2016_stats2015, dc2017_stats2016, dc2018_stats2017, 
                   dc2019_stats2018, dc2020_stats2019, dc2021_stats2020)
long_data <- long_data %>% 
  as.data.frame() %>% 
  filter(position == "WR")
```
...
```{r}
joined_data <- inner_join(long_data, combine_stats, 
                          by = c("player" = "Name"))
joined_data <- joined_data %>%
  mutate(Power5 = if_else(
    team_name_draft == "PITTSBURGH" |
      team_name_draft == "MICH STATE" |
      team_name_draft == "WASH STATE" |
      team_name_draft == "WISCONSIN" |
      team_name_draft == "ILLINOIS" |
      team_name_draft == "COLORADO" |
      team_name_draft == "TEXAS TECH" |
      team_name_draft == "UCLA" |
      team_name_draft == "OLE MISS" |
      team_name_draft == "OKLAHOMA" |
      team_name_draft == "BAYLOR" |
      team_name_draft == "S CAROLINA" |
      team_name_draft == "TCU" |
      team_name_draft == "BYU" |
      team_name_draft == "MISS STATE" |
      team_name_draft == "ARIZONA ST" |
      team_name_draft == "WASHINGTON" |
      team_name_draft == "ARIZONA" |
      team_name_draft == "GEORGIA" |
      team_name_draft == "OREGON" |
      team_name_draft == "CAL" |
      team_name_draft == "N CAROLINA" |
      team_name_draft == "MIAMI FL" |
      team_name_draft == "AUBURN" |
      team_name_draft == "DUKE" |
      team_name_draft == "CLEMSON" |
      team_name_draft == "NOTRE DAME" |
      team_name_draft == "FLORIDA" |
      team_name_draft == "IOWA STATE" |
      team_name_draft == "IOWA" |
      team_name_draft == "OHIO STATE" |
      team_name_draft == "RUTGERS" |
      team_name_draft == "BOSTON COL" |
      team_name_draft == "TENNESSEE" |
      team_name_draft == "SYRACUSE" |
      team_name_draft == "VA TECH" |
      team_name_draft == "USC" |
      team_name_draft == "MICHIGAN" |
      team_name_draft == "FLORIDA ST" |
      team_name_draft == "ARKANSAS" |
      team_name_draft == "TEXAS A&M" |
      team_name_draft == "W VIRGINIA" |
      team_name_draft == "PENN STATE" |
      team_name_draft == "UTAH" |
      team_name_draft == "ALABAMA" |
      team_name_draft == "LOUISVILLE" |
      team_name_draft == "LSU" |
      team_name_draft == "STANFORD" |
      team_name_draft == "OKLA STATE" |
      team_name_draft == "INDIANA" |
      team_name_draft == "MISSOURI" |
      team_name_draft == "KANSAS ST" |
      team_name_draft == "KANSAS" |
      team_name_draft == "WAKE" |
      team_name_draft == "NC STATE" |
      team_name_draft == "TEXAS" |
      team_name_draft == "MARYLAND" |
      team_name_draft == "MINNESOTA" |
      team_name_draft == "NEBRASKA" |
      team_name_draft == "NORTHWESTERN" |
      team_name_draft == "PURDUE" |
      team_name_draft == "VIRGINIA" |
      team_name_draft == "KENTUCKY" |
      team_name_draft == "VANDERBILT" |
      team_name_draft == "OREGON ST" |
      team_name_draft == "GA TECH",
    1, 0
  ))
joined_data <- joined_data %>% 
  distinct(player_id, .keep_all = TRUE)
  
write.csv(joined_data, "data/full_data.csv", row.names = TRUE)
head(joined_data)
```

```{r pro joined stats (test)}
pro2022stats <- read.csv("data/2022pronflstats.csv")
pro_joined <- inner_join(joined_data, pro2022stats, by = c("player", "player_id", "position"))
```
