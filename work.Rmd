...
train<-snap_model_coll_df[index,]
test<-snap_model_coll_df[-index,]
snap_model_coll  <- glm(playedA_Yr~., data=train, family = "binomial")
summary(snap_model_coll)
```

```{r snap model col}
set.seed(12321)
snap_model_coll_df2 <- snap_model_coll_df %>% 
  select(total_yards, total_catch_percent, total_target_percent, total_route_percent, total_drop_percent, total_yprr, total_first_downs, total_yards_per_reception, total_yards_after_catch_per_reception, total_player_game_count, total_touchdowns, total_yards_per_game, total_targets_per_game, total_targeted_qb_rating, games_draft, grades_hands_drop, grades_hands_fumble, grades_offense, grades_pass_route, Power5_draft, playedA_Yr)
index<-createDataPartition(snap_model_coll_df2$playedA_Yr,p=0.7,list=FALSE)
train<-snap_model_coll_df2[index,]
test<-snap_model_coll_df2[-index,]
snap_model_coll_2  <- glm(playedA_Yr~., data=train, family = "binomial")
summary(snap_model_coll_2)
```

```{r snap model 2}
set.seed(1)
snap_model_coll_df3 <- snap_model_coll_df %>% 
  select(total_yards, total_target_percent, total_drop_percent, total_yprr, total_yards_after_catch_per_reception, total_player_game_count, total_touchdowns, total_yards_per_game, total_targets_per_game, total_targeted_qb_rating, games_draft, grades_hands_drop, grades_offense, Power5_draft, playedA_Yr)
index<-createDataPartition(snap_model_coll_df2$playedA_Yr,p=0.7,list=FALSE)
train<-snap_model_coll_df3[index,]
test<-snap_model_coll_df3[-index,]
snap_model_coll_3  <- glm(playedA_Yr~., data=train, family = "binomial")
summary(snap_model_coll_3)

pred_snap <- predict(snap_model_coll_3, test, type = "response")
pred_snap <- as.integer(pred_snap>0.5)
conf_snap <- confusionMatrix(as.factor(pred_snap),test$playedA_Yr)
conf_snap
```

```{r decent starter model 1}
set.seed(1)
decent_starter_coll_df2 <- decent_starter_coll_df %>% 
  select(total_yards, total_catch_percent, total_target_percent, total_route_percent, total_drop_percent, total_yprr, total_first_downs, total_yards_per_reception, total_yards_after_catch_per_reception, total_player_game_count, total_touchdowns, total_yards_per_game, total_targets_per_game, total_targeted_qb_rating, games_draft, grades_hands_drop, grades_hands_fumble, grades_offense, grades_pass_route, Power5_draft, decentStarter)
index<-createDataPartition(decent_starter_coll_df2$decentStarter,p=0.7,list=FALSE)
train<-decent_starter_coll_df2[index,]
test<-decent_starter_coll_df2[-index,]
decent_starter_coll_df2 <- glm(decentStarter~., data=train, family = "binomial")
summary(decent_starter_coll_df2)
```

```{r decent starter model 2}
set.seed(1)
decent_starter_coll_df2 <- decent_starter_coll_df %>% 
  select(total_yards, total_target_percent, total_drop_percent, total_yprr, total_yards_after_catch_per_reception, total_player_game_count, total_touchdowns, total_yards_per_game, total_targets_per_game, total_targeted_qb_rating, grades_hands_drop, grades_offense, Power5_draft, decentStarter)
index<-createDataPartition(decent_starter_coll_df2$decentStarter,p=0.7,list=FALSE)
train<-decent_starter_coll_df2[index,]
test<-decent_starter_coll_df2[-index,]
decent_starter_coll_2 <- glm(decentStarter~., data=train, family = "binomial")
summary(decent_starter_coll_2)

pred_decent <- predict(decent_starter_coll_2, test, type = "response")
pred_decent <- as.integer(pred_snap>0.5)
conf_decent <- confusionMatrix(as.factor(pred_decent),test$decentStarter)
conf_decent
```

```{r great starter model 1}
set.seed(1)
great_starter_coll_df2 <- great_starter_coll_df %>% 
  select(total_yards, total_target_percent, total_route_percent, total_drop_percent, total_yprr, total_first_downs, total_yards_per_reception, total_yards_after_catch_per_reception, total_player_game_count, total_touchdowns, total_yards_per_game, total_targets_per_game, total_targeted_qb_rating, grades_hands_drop, grades_hands_fumble, grades_offense, grades_pass_route, Power5_draft, greatStarter)
index<-createDataPartition(great_starter_coll_df2$greatStarter,p=0.7,list=FALSE)
train<-great_starter_coll_df2[index,]
test<-great_starter_coll_df2[-index,]
great_starter_coll_1 <- glm(greatStarter~., data=train, family = "binomial")
summary(great_starter_coll_1)
```

```{r great starter model 2}
set.seed(1)
great_starter_coll_df2 <- great_starter_coll_df %>% 
  select(total_yards, total_target_percent, total_drop_percent, total_yprr, total_yards_after_catch_per_reception, total_player_game_count, total_touchdowns, total_yards_per_game, total_targets_per_game, total_targeted_qb_rating, grades_hands_drop, grades_offense, Power5_draft, greatStarter)
index<-createDataPartition(great_starter_coll_df2$greatStarter,p=0.7,list=FALSE)
train<-great_starter_coll_df2[index,]
test<-great_starter_coll_df2[-index,]
great_starter_coll_2  <- glm(greatStarter~., data=train, family = "binomial")
summary(great_starter_coll_2)

pred_great <- predict(great_starter_coll_2, test, type = "response")
pred_great <- as.integer(pred_great>0.50)
conf_great <- confusionMatrix(as.factor(pred_decent),test$greatStarter)
conf_great
```
